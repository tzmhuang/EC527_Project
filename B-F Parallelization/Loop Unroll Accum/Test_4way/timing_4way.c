...
void BellmanFord_LUAC(struct Graph *graph, int index, data_t** distance)
{
    ...
    data_t weight0, weight1, weight2, weight3, weight4, weight5, testDist0, testDist1, testDist2, testDist3, iDist;
    data_t *StoreDistance = (data_t *)calloc(V, sizeof(data_t));
    int i, j, n;
    int source = index - 1;
...
int main()
{
    ...
    */
    ...
    clock_gettime(CLOCK_REALTIME, &time_stop);
    elapse_time = interval(time_start, time_stop);
    printf("\nTime for MODIFIED Bellman-Ford (4-way) for each keypoint: %f(msec)\n", (float)(elapse_time));
