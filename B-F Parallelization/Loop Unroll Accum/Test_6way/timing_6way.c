...
// Basic Bellman-Ford with Loop Unrolling + Accumulators
void BellmanFord_LUAC(struct Graph *graph, int index, data_t** distance)
{
    int V = graph->Ve;
    int to0, to1, to2, to3, to4, to5, nodes, nodesRemain;
    struct AdjListNode *from0 = NULL;
    struct AdjListNode *from1 = NULL;
    struct AdjListNode *from2 = NULL;
    struct AdjListNode *from3 = NULL;
    struct AdjListNode *from4 = NULL;
    struct AdjListNode *from5 = NULL;
    data_t weight0, weight1, weight2, weight3, weight4, weight5, testDist0, testDist1, testDist2, testDist3, testDist4, testDist5, iDist;
    data_t *StoreDistance = (data_t *)calloc(V, sizeof(data_t));
    int i, j, n;
    int source = index - 1;
    ...
    from0 = graph->array[source].head;
    nodes = graph->array[source].nodeCount;
    for (i = 0; i < nodes; i++) {
        to0 = from0->dest;
        weight0 = from0->dist;
        if (StoreDistance[to0] > StoreDistance[source] + weight0)
        {
            StoreDistance[to0] = StoreDistance[source] + weight0;
        }
        from0 = from0->next;
    }
    ...
    for (n = 0; n < V - 1; n++)
    {
        for (i = 0; i < V; i++)
        {
            iDist = StoreDistance[i];
            nodes = graph->array[i].nodeCount;
            nodesRemain = nodes;
            from0 = graph->array[i].head;
            from1 = from0->next;
            // printf("In LUAC, 3a (nodes: %d)\n", nodes);
            if (nodesRemain > 5) {
                from2 = from1->next;
                from3 = from2->next;
                from4 = from3->next;
                from5 = from4->next;
                for (j = 0; j < nodes-5; j+=6) {
                    to0 = from0->dest;
                    weight0 = from0->dist;
                    to1 = from1->dest;
                    weight1 = from1->dist;
                    ...
                    to5 = from5->dest;
                    weight5 = from5->dist;
                    // printf("In LUAC, 3a1 (weight0: %f, weight1: %f)\n", weight0, weight1);
                    testDist0 = iDist + weight0;
                    testDist1 = iDist + weight1;
                    testDist2 = iDist + weight2;
                    testDist3 = iDist + weight3;
                    testDist4 = iDist + weight4;
                    testDist5 = iDist + weight5;
                    if (StoreDistance[to0] > testDist0) StoreDistance[to0] = testDist0;
                    if (StoreDistance[to1] > testDist1) StoreDistance[to1] = testDist1;
                    if (StoreDistance[to2] > testDist2) StoreDistance[to2] = testDist2;
                    if (StoreDistance[to3] > testDist3) StoreDistance[to3] = testDist3;
                    if (StoreDistance[to4] > testDist4) StoreDistance[to4] = testDist4;
                    if (StoreDistance[to5] > testDist5) StoreDistance[to5] = testDist5;
                    nodesRemain-=6;
                    if (nodesRemain > 0) from0=from5->next;
                    // printf("In LUAC, 3a2\n");
                    if (nodesRemain > 1) from1=from0->next;
                    if (nodesRemain > 2) from2=from1->next;
                    if (nodesRemain > 3) from3=from2->next;
                    if (nodesRemain > 4) from4=from3->next;
                    if (nodesRemain > 5) from5=from4->next;
                    // printf("In LUAC, 3a3\n");
                }
            }
            if (nodesRemain > 3) {
                ...
                if (StoreDistance[to3] > testDist3) StoreDistance[to3] = testDist3;
                nodesRemain-=4;
                if (nodesRemain > 0) from0=from3->next;
                // // printf("In LUAC, 3a2\n");
                // if (nodesRemain > 1) from1=from0->next;
                // if (nodesRemain > 2) from2=from1->next;
                // if (nodesRemain > 3) from3=from2->next;
            ...
            if (nodesRemain == 3) {
                ...
                if (StoreDistance[to2] > testDist2) StoreDistance[to2] = testDist2;
                nodesRemain-=3;
                // if (nodesRemain > 0) from0=from2->next;
                // // printf("In LUAC, 3a2\n");
                // if (nodesRemain > 1) from1=from0->next;
                // if (nodesRemain > 2) from2=from1->next;
                // printf("In LUAC, 3a3\n");
            ...
            // printf("In LUAC, 3b (nodes: %d, nodesRemain: %d)\n", nodes, nodesRemain);
            if (nodesRemain == 1) {
                to0 = from0->dest;
                weight0 = from0->dist;
                testDist0 = iDist + weight0;
...
void output_obj(data_t *verts, int *face_nearest_kp, int *faces, int Ve, int F, char *filename) {
...
int main()
{
    ...
    */
    ...
    clock_gettime(CLOCK_REALTIME, &time_stop);
    elapse_time = interval(time_start, time_stop);
    printf("\nTime for adding edges to graph: %f(msec)\n", (float)(elapse_time));
    ...
    */
    ...
    /***********************************************************************************************
    Step6a: Run MODIFIED Bellman-Ford for each keypoint
    */
    ...
    // Keypoint id = [Ve, Ve+Kp)
    data_t* dist_arrayA;
    data_t* vk_shortest_distA = (data_t *)calloc(Ve*Kp, sizeof(data_t)); // Ve-by-Kp array
    for (int k=Ve; k<Ve+Kp; k++){
        BellmanFord_LUAC(graph, k+1, &dist_arrayA); // id starts from 1
        // PrintDistance(dist_array, Ve+Kp, k+1);
        for(int v=0; v<Ve;v++){
            vk_shortest_distA[v*Kp + k-Ve] = dist_arrayA[v]; // copy result into array (alt.: use ptr)
        }
    }

    clock_gettime(CLOCK_REALTIME, &time_stop);
    elapse_time = interval(time_start, time_stop);
    printf("\nTime for MODIFIED Bellman-Ford (6-way) for each keypoint: %f(msec)\n", (float)(elapse_time));
    ...
    */
    ...
    /***********************************************************************************************
    Step7a: Find closest Kp for all Vt from MODIFIED B-F
    */

    clock_gettime(CLOCK_REALTIME, &time_start);
    // Find closest Kp for all Vt
    int *nearest_kpA = (int *)malloc(Ve * sizeof(int));
    data_t min_dA, dA;
    int min_idA;
    for(int i=0; i<Ve; i++){
        min_dA = INT_MAX;
        min_idA=0;
        for(int j=0; j<Kp; j++){
            dA = vk_shortest_distA[i*Kp + j];
            if (dA < min_dA){
                min_dA = dA;
                min_idA = j;
            }
        }
        nearest_kpA[i] = min_idA;
    }


    clock_gettime(CLOCK_REALTIME, &time_stop);
    elapse_time = interval(time_start, time_stop);
    printf("\nTime for Find closest Kp for all Vts in MODIFIED B-F: %f(msec)\n", (float)(elapse_time));
    ...
    */
    ...
    /***********************************************************************************************
    Step8: Find closest Kp for all Faces from MODIFIED B-F
    */

    clock_gettime(CLOCK_REALTIME, &time_start);

    int *face_nearest_kpA = (int *)malloc(F * sizeof(int));
    faces_kp(face_nearest_kpA, faces, nearest_kpA, F);
    // for (int i = 0; i < F; i++)
    // {
    //     printf("The shortest keypoint from face %d is: %d \n", i, face_nearest_kp[i]);
    // }
    ...
    clock_gettime(CLOCK_REALTIME, &time_stop);
    elapse_time = interval(time_start, time_stop);
    printf("\nTime for Find closest Kp for all Faces in MODIFIED B-F: %f(msec)\n", (float)(elapse_time));


    // Output the updated obj file
    output_obj(verts, face_nearest_kp, faces, Ve, F, "segmented_child.obj");
    output_obj(verts, face_nearest_kpA, faces, Ve, F, "segmented_child_luac.obj");
